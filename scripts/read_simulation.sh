...
wgsim -1 $read_length -2 $read_length -N $read_pairs -r $mutation_rate -R $indel_fraction -X $indel_extension -e $error_rate ${mat_hap1} ${tmpdir}/mat_hap1_R1.fastq ${tmpdir}/mat_hap1_R2.fastq
wgsim -1 $read_length -2 $read_length -N $read_pairs -r $mutation_rate -R $indel_fraction -X $indel_extension -e $error_rate ${mat_hap2} ${tmpdir}/mat_hap2_R1.fastq ${tmpdir}/mat_hap2_R2.fastq

cat ${tmpdir}/mat_hap1_R1.fastq ${tmpdir}/mat_hap1_R2.fastq ${tmpdir}/mat_hap2_R1.fastq ${tmpdir}/mat_hap2_R2.fastq | gzip > ${out_mat}
